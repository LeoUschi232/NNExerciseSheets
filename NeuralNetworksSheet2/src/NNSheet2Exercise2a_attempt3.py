...
psi[0] = np.array([np.array([0.0 if (xn <= 0 or yn <= 0 or xn >= xmax or yn >= ymax)
                             else exp(kexp * (xsteps[xn] * xsteps[xn] + ysteps[yn] * ysteps[yn]))
                                  * sin(ksin * xsteps[xn]) * sin(ksin * ysteps[yn])
...
for yn in yrange:
    for xn in xrange:
        if fabs(psi[1, yn, xn]) > fabs(psi[0, yn, xn]) \
                and np.sign(psi[0, yn, xn]) == np.sign(psi[1, yn, xn]):
            print(f"yn = {yn} and xn = {xn}\n{psi[1, yn, xn]} > {psi[0, yn, xn]}")
            exit(0)

for tn in range(1, tmax):
    psi[tn + 1] = np.array([np.array([0.0 if (xn <= 0 or yn <= 0 or xn >= xmax or yn >= ymax)
                                      else (multiplier * (
            (psi[tn, yn, xn + 1] + psi[tn, yn, xn - 1] - 2 * psi[tn, yn, xn]) / (dx * dx) +
            (psi[tn, yn + 1, xn] + psi[tn, yn - 1, xn] - 2 * psi[tn, yn, xn]) / (dy * dy))
